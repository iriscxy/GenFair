...
class Transformer(nn.Module):
    ...
    def decode(self, hidden_states, src_mask, tgt, tgt_mask):
        if tgt.size(0)!=hidden_states.size(0):
            size=int(tgt.size(0)/hidden_states.size(0))
            hidden_states=torch.repeat_interleave(hidden_states, size, dim=0)
            src_mask=torch.repeat_interleave(src_mask, size, dim=0)
