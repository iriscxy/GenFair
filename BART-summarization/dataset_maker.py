    ...
    def make_dataset(self):
        ...
        def preprocess_function(examples: Dict):
            ...
            if isinstance(examples["src"][0], str):
                ...
                inputs = [' '.join(ex).replace(' ', '') if self.data_args.chinese_data else ' '.join(ex) for ex in examples["src"]]
            else:
                raise ValueError(f'only support str/list in content, now {type(examples["src"][0])}')

            if isinstance(examples["impression"][0], str):
                targets = [ex.replace(' ', '') + self.tokenizer.eos_token if self.data_args.chinese_data else ex + self.tokenizer.eos_token for ex in examples["impression"]]
            elif isinstance(examples["impression"][0], list):
                targets = [' '.join(ex).replace(' ', '') + self.tokenizer.eos_token if self.data_args.chinese_data else ' '.join
                    (ex) + self.tokenizer.eos_token for ex in examples["impression"]]
            else:
                raise ValueError(f'only support str/list in summary, now {type(examples["impression"][0])}')
